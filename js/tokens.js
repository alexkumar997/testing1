...
var _syncInFlight     = null;   // In-flight fetchAndSyncTokens Promise — deduplicates concurrent callers
...
//
// _doFetchAndSync contains the real logic.
// fetchAndSyncTokens is the public entry point — it deduplicates concurrent
// callers so that only ONE network request is ever in-flight at a time.
async function _doFetchAndSync() {
  ...
  try {
    ...
    if (isNewISTDay && istHour >= 4) {
      ...
      // E.g. app opened at IST 8:18 AM → 5 missed hours → 5 × 200 = 1000 tokens.
    ...
    if (istHour >= 4 && istHour <= 22) {
      ...
      if (hoursMissed >= 1) {
        var refillPerHour = pl.hourly_refill     || 200;
        var dailyLimit    = pl.total_daily_limit || 3800;
...
// If a sync is already in-flight, all concurrent callers share that one promise
// instead of each firing their own Supabase request.
async function fetchAndSyncTokens() {
  if (_syncInFlight) return _syncInFlight;
  _syncInFlight = _doFetchAndSync();
...
async function checkPlayAllowed() {
  ...
  if (pl.used_today >= (pl.total_daily_limit || 3800)) return 'daily_limit';
...
async function deductPlayToken() {
  ...
  if (_tokenDeducting) {
    ...
    if (_lastKnownTokens) {
      if (_lastKnownTokens.used_today >= (_lastKnownTokens.total_daily_limit || 3800))
        return 'daily_limit';
  ...
  try {
    ...
    // ── Check daily limit ──
    if (pl.used_today >= (pl.total_daily_limit || 3800)) {
...
function startTokenEngine() {
  ...
  // 2. Periodic background sync every 20 minutes — cache-first:
  //    skip the network call if the cache was refreshed recently (< 5 min ago)
  //    so that a deduction or pre-play check that just ran isn't immediately
  //    duplicated when the timer happens to fire shortly after.
  if (_engineTimer) clearInterval(_engineTimer);
  _engineTimer = setInterval(function() {
    var cacheAgeMs = Date.now() - _cacheTimestamp;
    if (_lastKnownTokens && cacheAgeMs < 300000) {
      console.log('[ENGINE] Periodic sync skipped — cache is ' +
        Math.round(cacheAgeMs / 1000) + 's old');
      return;
    }
...
async function initPolicyPage() {
  ...
  // Fresh sync — cache-first: skip the round-trip if data is < 2 min old.
  // The Realtime subscription will push any changes that arrive while the
  // policy page is open, so a redundant fetch here wastes egress for nothing.
  var cacheAgeMs = Date.now() - _cacheTimestamp;
  if (!_lastKnownTokens || cacheAgeMs > 120000) {
...
function updatePolicyTokenDisplay(data) {
  ...
  var dailyMax  = (data.total_daily_limit !== undefined) ? data.total_daily_limit : 3800;
  ...
  if (usedToday >= dailyMax) {
    ...
    statusEl.textContent = 'Live · Tokens available · +' + (data.hourly_refill || 200) +
